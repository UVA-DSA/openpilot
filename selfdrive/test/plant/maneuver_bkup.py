...
class Maneuver(object):
  ...
  def evaluate(self, output_dir):
    ...
    visionLane = 0  # use path model from matlab output
    angle_steer = 0.0
    prev_distance = 0.0
    ...
    while plant.current_time() < self.duration:
      ...
      distance, speed, acceleration, distance_lead, brake, gas, steer_torque, live100, angle_steer, reportHazard = plant.step(reportHazard, outfile, hazardfile, speed_lead, current_button, grade, self.frameIdx, self.pathOffset, self.lLane, self.rLane, delta_lane) # angle_steer (return parameter), outfile, hazardfile, self.frameIdx, self.pathOffset, lLane, rLane added by Hasnat
